...
curl -X POST "https://identitytoolkit.googleapis.com/v2/projects/$ENV_FIREBASE_APP_ID/defaultSupportedIdpConfigs?idpId=facebook.com" \
  -H "Authorization: Bearer $TOKEN" \
  -H 'Content-Type: application/json' \
  -d "{ name : \"projects/$ENV_FIREBASE_APP_ID/defaultSupportedIdpConfigs/facebook.com\", enabled : true, clientId : \"$ENV_FACEBOOK_APP_ID\", clientSecret : \"$ENV_FACEBOOK_APP_SECRET\" }"

curl -X POST "https://firebase.googleapis.com/v1beta1/projects/$ENV_FIREBASE_APP_ID/androidApps" \
  -H "Authorization: Bearer $TOKEN" \
  -H 'Content-Type: application/json' \
  -d '{"packageName": "com.chaomao.stock" }'

curl -X POST "https://firebase.googleapis.com/v1beta1/projects/$ENV_FIREBASE_APP_ID/iosApps" \
  -H "Authorization: Bearer $TOKEN" \
  -H 'Content-Type: application/json' \
...
echo "$DEBUG_KEY"
curl -X POST "https://firebase.googleapis.com/v1beta1/projects/$ENV_FIREBASE_APP_ID/androidApps/$ANDROID_APP_ID/sha" \
  -H "Authorization: Bearer $TOKEN" \
  -H 'Content-Type: application/json' \
